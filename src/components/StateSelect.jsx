...
import { HERO_STATE_OPTIONS } from "../lib/properties/constants";
      ...
      <Listbox value={value} onChange={onChange}>
        ...
        >
          <Listbox.Options className="absolute z-50 mt-4 max-h-64 w-full overflow-auto rounded-lg border border-slate-200 bg-white py-2 shadow-xl">
            {HERO_STATE_OPTIONS.map((state) => (
              <Listbox.Option key={state} value={state}>
                {({ active, selected }) => (
                  <div
                    className={`cursor-pointer px-4 py-1 text-sm transition ${
                      active ? "bg-primary-subtle" : ""
                    } ${selected ? "font-semibold text-primary" : "text-slate-500"}`}
                  >
                    {state}
                  </div>
                )}
